...
import type { Seat, SeatVersion2 } from '../types/theater.interface';
import { seatPrices } from '../types/booking.interface';
import { setBooking } from '../redux/slice/booking.slice';
import { getPaymentByShowTimeId } from '../api/payment.api';
...
export default function ChooseTicket() {
    ...
    const [choosingSeat, setChoosingSeat] = useState<SeatVersion2[]>([]);
    ...
    const paymentHistory = useAppSelector((s) => s.payment.payments);
            ...
            <div className="h-auto px-8 bg-gray-900 opacity-80 flex flex-col justify-center items-center gap-5 py-10">
                <p className="text-lg text-orange-400 text-center">
                    <span className="font-semibold text-[15px]">
                        Lưu ý: Khán giả dưới 13 tuổi chỉ chọn suất chiếu kết
                        thúc trước 22h và Khán giả dưới 16 tuổi chỉ chọn suất
                ...
                <div className={`${showing ? '' : 'hidden'}`}>
                    ...
                    `}
                                    >
                                        <Armchair
                                            ...
                                            className={`
                                                ...
                                                ${
                                                    ghe.locked
                                                        ? 'text-red-500' // 🔒 ghế đã lock (API)
                                                        : ghe.booked
                                                        ? 'text-[#007AFF]' // 🟦 ghế bạn chọn
                                                        : ghe.type ===
                                                          'standard'
                                                        ? 'text-white' // ⚪ ghế thường
                                                        : ghe.type === 'vip'
                                                        ? 'text-amber-300' // 🟨 ghế VIP
                                                        : ghe.type ===
                                                          'sweetbox'
                                                        ? 'text-pink-500' // 💗 ghế đôi
                                                        : ''
                                                }
                                            `}
                                        />
                            ...
                            <div className="flex justify-center gap-6 text-xs mb-4 items-center text-[16px]">
                                ...
                                Đã đặt
                                <div className="size-10 rounded-md bg-[#007AFF]"></div>{' '}
                                Ghế bạn chọn
                                <div className="size-10 rounded-md bg-white"></div>{' '}
                                Ghế thường
                                <div className="bg-amber-300 size-10 rounded-md"></div>{' '}
                                Ghế VIP
                                <div className="bg-pink-500 size-10 rounded-md"></div>{' '}
                                Ghế đôi
                            </div>
                        </div>
                        ...
                        <div className="text-right">
                            ...
                            <button
                                className="bg-red-600 px-4 py-2 rounded hover:bg-red-700 transition-colors duration-300 hover:scale-105 cursor-pointer"
                                onClick={() => {
                                    dispatch(
                                        setBooking({
                                            nameFilm: showTimeNow?.movie || '',
                                            showtime: `${showTimeNow?.date} ${hour}`,
                                            nameScreen:
                                                showTimeNow?.screen || '',
                                            type: '2D',
                                            seats: choosingSeat,
                                            showTimeId: showTimeNow?.id || '',
                                            userId: user ? user.id : '',
                                        })
                                    );
                                    navigate('/payment');
