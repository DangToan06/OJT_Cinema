...
export function SeatsManagement() {
  ...
  const screenList = useAppSelector((s) => s.screens.screens);
  const seatMapList = useAppSelector((s) => s.seat.listMapSeatsEntity);
  ...
  useEffect(() => {
    if (seatMapList.length === 0) {
      dispatch(getAllSeatsMap());
    }
  }, [dispatch, seatMapList.length]);
    ...
    setSeats((prev: SeatsMap) => ({
      ...
      [selectedScreenId]: prev[selectedScreenId].map((seat: Seat) => {
        if (seat.row === row && seat.number === number) {
          const types: SeatType[] = ["standard", "vip", "sweetbox", "disabled"];
  ...
  const saveMapSeats = async () => {
    if (!selectedScreenId) return;

    const currentSeats = seats[selectedScreenId];
    if (!currentSeats || currentSeats.length === 0) return;

    const existed = seatMapList.find(
      (item) => item.screenId === selectedScreenId
    );
    ...
    if (existed) {
      dispatch(
        updateSeatsMap({
          id: existed.id,
          screenId: existed.screenId,
          seats: currentSeats,
        })
      );
      notify.success("Cập nhật sơ đồ ghế thành công!");
    } else {
      dispatch(
        createSeatsMap({
          id: uuidv4(),
  ...
  const resetAllSeats = () => {
    if (!selectedScreenId || !currentScreen) return;

    const resetSeats: Seat[] = [];
    for (let r = 0; r < currentScreen.row; r++) {
      for (let c = 0; c < currentScreen.column; c++) {
        resetSeats.push({
          row: String.fromCharCode(65 + r),
          number: c + 1,
          type: "standard",
    ...
    setSeats((prev) => ({
      ...prev,
      [selectedScreenId]: resetSeats,
    }));

    notify.success("Đã reset tất cả ghế về mặc định!");
  };
        ...
        <div className="lg:col-span-1 space-y-6">
          ...
          <div className="bg-linear-to-br from-slate-800/60 to-slate-900/60 backdrop-blur-xl border border-slate-700/50 rounded-2xl p-5">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="text-slate-100 font-semibold">Chọn rạp chiếu</h3>
            </div>
          ...
          {selectedTheaterId && (
            <div className="bg-linear-to-br from-slate-800/60 to-slate-900/60 backdrop-blur-xl border border-slate-700/50 rounded-2xl p-5">
              ...
              {screenList.filter(
                (screen) => screen.theaterId === selectedTheaterId
              ).length === 0 ? (
                /* EMPTY STATE */
                <div className="flex flex-col items-center justify-center py-10 text-slate-400">
                  <Film className="w-12 h-12 mb-3 opacity-60" />
                  <p className="text-sm">Rạp này chưa có phòng chiếu</p>
                </div>
              ) : (
                /* LIST SCREEN */
                <div className="space-y-2 max-h-80 overflow-y-auto pr-2 custom-scrollbar">
                  {screenList
                    .filter((screen) => screen.theaterId === selectedTheaterId)
                    .map((screen) => (
                      <button
                        key={screen.id}
                        onClick={() => {
                          setSelectedScreenId(screen.id);
                          loadSeatsForScreen(screen.id);
                        }}
                        className={`w-full px-4 py-3 rounded-xl text-left transition-all cursor-pointer ${
                      ...
                      >
                        ...
                        >
                          {screen.row} hàng x {screen.column} ghế
                        </p>
                      </button>
                    ))}
                </div>
              ...
              {seatTypes.map((type) => (
                <div key={type.id} className="flex items-center gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-slate-500 text-xs">{type.price} ₫</p>
                  </div>
                </div>
              ))}
            </div>
              ...
              <>
                ...
                <div className="overflow-x-auto pb-4">
                  <div className="inline-flex flex-col items-center gap-3 min-w-full justify-center">
                    {[...Array(currentScreen.row)].map((_, rowIndex) => {
                      ...
                      const seatSize = getSeatSize(currentScreen.column);
                      const iconSize = getIconSize(currentScreen.column);
                      const gapSize = getGapSize(currentScreen.column);
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className={`flex ${gapSize}`}>
                            {[...Array(currentScreen.column)].map(
                              (_, seatIndex) => {
                ...
                <div className="mt-8 pt-6 border-t border-slate-700/50">
                  <div className="grid grid-cols-2 md:grid-cols-5 gap-4 text-sm">
                    <div className="bg-slate-900/50 rounded-xl p-3 border border-slate-700/30">
                      <p className="text-slate-500 text-xs mb-1">Tổng số ghế</p>
                      <p className="text-slate-100 font-bold text-lg">
                        {currentScreen.row * currentScreen.column}
                      </p>
                    </div>
                    <div className="bg-slate-900/50 rounded-xl p-3 border border-slate-700/30">
                      <p className="text-slate-500 text-xs mb-1">Standard</p>
                      <p className="text-slate-100 font-bold text-lg">
                        {
                          currentSeats.filter(
                            (s: Seat) => s.type === "standard"
                          ).length
                        }
                      </p>
                    </div>
                    <div className="bg-slate-900/50 rounded-xl p-3 border border-slate-700/30">
                      <p className="text-slate-500 text-xs mb-1">VIP</p>
                      <p className="text-amber-400 font-bold text-lg">
                        {
                          currentSeats.filter((s: Seat) => s.type === "vip")
                            .length
                        }
                      </p>
                    </div>
                    <div className="bg-slate-900/50 rounded-xl p-3 border border-slate-700/30">
                      <p className="text-slate-500 text-xs mb-1">Sweetbox</p>
                      <p className="text-pink-400 font-bold text-lg">
                        {
                          currentSeats.filter(
                            (s: Seat) => s.type === "sweetbox"
                          ).length
                        }
                      </p>
                    </div>
                    <div className="bg-slate-900/50 rounded-xl p-3 border border-slate-700/30">
                      <p className="text-slate-500 text-xs mb-1">Ẩn/Hỏng</p>
                      <p className="text-slate-400 font-bold text-lg">
                        {
                          currentSeats.filter(
                            (s: Seat) => s.type === "disabled"
                          ).length
                        }
                      </p>
                    </div>
                ...
                <div className="mt-6 flex gap-3">
                  <button
                    ...
                    className="flex-1 flex items-center justify-center gap-2 px-6 py-3 border border-slate-600 text-slate-300 rounded-xl hover:bg-slate-700/30 transition-all font-medium active:scale-95"
                  >
                    <RotateCcw className="w-5 h-5" />
                    Đặt lại
                  </button>
                  <button
                    onClick={saveMapSeats}
                    className="flex-1 flex items-center justify-center gap-2 px-6 py-3 bg-linear-to-r from-rose-600 to-pink-600 hover:from-rose-700 hover:to-pink-700 text-white rounded-xl transition-all hover:scale-105 hover:shadow-lg hover:shadow-rose-500/30 font-medium"
                  >
