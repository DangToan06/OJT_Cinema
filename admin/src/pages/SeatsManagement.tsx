...
export function SeatsManagement() {
    ...
    const seatTypes = [
        { id: 'standard', label: 'Standard', color: 'bg-slate-500', price: '75,000' },
        { id: 'vip', label: 'VIP', color: 'bg-amber-500', price: '120,000' },
        { id: 'sweetbox', label: 'Sweetbox', color: 'bg-pink-500', price: '200,000' },
        ...
        screenList.forEach((screen) => {
            ...
            for (let row = 0; row < screen.row; row++) {
                for (let seat = 0; seat < screen.column; seat++) {
                    ...
                    const isSweetbox = row === screen.row - 1 && seat >= 5 && seat <= 6;
                    const isDisabled = (row === 2 && seat === 5) || (row === 7 && seat === 9);
                            ...
                            {theaterList.map((theater) => (
                                ...
                                >
                                    <p className={`font-medium ${
                                        selectedTheaterId === theater.id ? 'text-white' : 'text-slate-200'
                                    }`}>
                                        {theater.name}
                                    </p>
                                    <p className={`text-xs mt-1 ${
                                        selectedTheaterId === theater.id ? 'text-rose-100' : 'text-slate-500'
                                    }`}>
                                        {theater.address}
                                    </p>
                                </button>
                    ...
                    {selectedTheaterId && (
                        <div className="bg-linear-to-br from-slate-800/60 to-slate-900/60 backdrop-blur-xl border border-slate-700/50 rounded-2xl p-5">
                            <div className="flex items-center gap-2 mb-4">
                                ...
                                <h3 className="text-slate-100 font-semibold">Chọn phòng chiếu</h3>
                            </div>
                            <div className="space-y-2 max-h-80 overflow-y-auto pr-2 custom-scrollbar">
                                {screenList
                                    ...
                                    .map((screen) => (
                                        ...
                                        >
                                            <p className={`font-medium ${
                                                selectedScreenId === screen.id ? 'text-white' : 'text-slate-200'
                                            }`}>
                                                {screen.name}
                                            </p>
                                            <p className={`text-xs mt-1 ${
                                                selectedScreenId === screen.id ? 'text-blue-100' : 'text-slate-500'
                                            }`}>
                                                {screen.row} hàng × {screen.column} ghế
                                            </p>
                                        </button>
                                    ))}
                            </div>
                        ...
                        <div className="space-y-3">
                            {seatTypes.map((type) => (
                                <div key={type.id} className="flex items-center gap-3">
                                    <div className={`w-8 h-8 rounded-lg ${type.color} flex items-center justify-center shadow-md`}>
                                        <Armchair className="w-4 h-4 text-white" />
                                    </div>
                                    <div className="flex-1">
                                        <p className="text-slate-200 text-sm font-medium">{type.label}</p>
                    ...
                    <div className="bg-linear-to-br from-slate-800/60 to-slate-900/60 backdrop-blur-xl border border-slate-700/50 rounded-2xl p-6">
                        {currentScreen ? (
                                ...
                                <div className="mb-8">
                                    <div className="relative">
                                        <div className="bg-linear-to-b from-slate-700 via-slate-600 to-slate-500 text-white text-center py-4 rounded-2xl shadow-2xl mb-3">
                                            <p className="text-sm font-bold tracking-widest">MÀN HÌNH CHIẾU</p>
                                        </div>
                                            ...
                                            return (
                                                <div key={rowLetter} className="flex items-center gap-3">
                                                    ...
                                                    <div className={`flex ${gapSize}`}>
                                                        {[...Array(currentScreen.column)].map((_, seatIndex) => {
                                                            const seatNumber = seatIndex + 1;
                                                            const seat = currentSeats.find(
                                                                (s: Seat) => s.row === rowLetter && s.number === seatNumber
                                                            );

                                                            return (
                                                                <button
                                                                    key={seatNumber}
                                                                    onClick={() => toggleSeatType(rowLetter, seatNumber)}
                                                                    className={`${seatSize} rounded-t-xl flex items-center justify-center transition-all hover:scale-110 hover:shadow-lg active:scale-95 ${
                                                                        seat ? getSeatColor(seat.type) : 'bg-slate-500'
                                                                    }`}
                                                                    title={`${rowLetter}${seatNumber} - ${
                                                                        seatTypes.find((t) => t.id === seat?.type)?.label || 'Standard'
                                                                    }`}
                                                                >
                                                                    <Armchair className={`${iconSize} text-white`} />
                                                                </button>
                                                            );
                                                        })}
                                                    </div>
                                ...
                                <div className="mt-8 pt-6 border-t border-slate-700/50">
                                    <div className="grid grid-cols-2 md:grid-cols-5 gap-4 text-sm">
                                        ...
                                        <div className="bg-slate-900/50 rounded-xl p-3 border border-slate-700/30">
                                            ...
                                            <p className="text-slate-100 font-bold text-lg">
                                                {currentSeats.filter((s: Seat) => s.type === 'standard').length}
                                            </p>
                                        </div>
                                        <div className="bg-slate-900/50 rounded-xl p-3 border border-slate-700/30">
                                            <p className="text-slate-500 text-xs mb-1">VIP</p>
                                            <p className="text-amber-400 font-bold text-lg">
                                                {currentSeats.filter((s: Seat) => s.type === 'vip').length}
                                            </p>
                                        </div>
                                        <div className="bg-slate-900/50 rounded-xl p-3 border border-slate-700/30">
                                            <p className="text-slate-500 text-xs mb-1">Sweetbox</p>
                                            <p className="text-pink-400 font-bold text-lg">
                                                {currentSeats.filter((s: Seat) => s.type === 'sweetbox').length}
                                            </p>
                                        </div>
                                        <div className="bg-slate-900/50 rounded-xl p-3 border border-slate-700/30">
                                            <p className="text-slate-500 text-xs mb-1">Ẩn/Hỏng</p>
                                            <p className="text-slate-400 font-bold text-lg">
                                                {currentSeats.filter((s: Seat) => s.type === 'disabled').length}
                                            </p>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div className="flex flex-col items-center justify-center py-20">
                                ...
                                <p className="text-slate-400 font-medium text-lg mb-2">Chưa chọn phòng chiếu</p>
                                <p className="text-slate-500 text-sm">Vui lòng chọn rạp và phòng chiếu để bắt đầu</p>
                            </div>
                        )}
                    </div>
                </div>
